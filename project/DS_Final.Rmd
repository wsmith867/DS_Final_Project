...
To help avoid errors going forward, we will be renaming `Prime Video` and `Disney+`.
```{r}
platforms <- platforms %>% 
...
## Distribution of target Age for each platform
...
How recent are the movies on each platform?  Would newer platforms such as Disney+ have the most?
```{r}
platforms %>% 
  gather(Netflix, Hulu, PrimeVideo, DisneyP, key = "platform", value = "value") %>% 
  group_by(platform) %>% 
